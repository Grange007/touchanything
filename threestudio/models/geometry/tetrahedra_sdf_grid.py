...
class TetrahedraSDFGrid(BaseExplicitGeometry):
    ...
    class Config(BaseExplicitGeometry.Config):
        ...
        # Extract surface from `sdf == isosurface_threshold` (threshold=0 means zero level-set).
        ...
        # Scale factor applied when converting a previous-stage mesh into this grid.
        # If set, it overrides nerf_scale for conversion.
        convert_mesh_scale: Optional[float] = None
    ...
    def initialize_shape(self) -> None:
        ...
        if self.cfg.shape_init == "ellipsoid":
            ...
            print(f"Initializing shape from mesh: {mesh_path}")
        ...
        elif self.cfg.shape_init.startswith("pointcloud:"):
            assert isinstance(self.cfg.shape_init_params, float)
            pc_path = self.cfg.shape_init[11:]
            print(f"Initializing shape from point cloud: {pc_path}")
            if not os.path.exists(pc_path):
                raise ValueError(f"Point cloud file {pc_path} does not exist.")

            import trimesh
            import numpy as np

            # 加载点云数据
            if pc_path.endswith('.ply') or pc_path.endswith('.pcd'):
                points = trimesh.load(pc_path).vertices
            else:
                points = np.loadtxt(pc_path)  # 假设是txt格式
            
            # 从点云重建网格 - 使用凸包作为简单方法
            # 注意：这只是一个基础实现，可能需要更复杂的重建算法
            ...
            if mesh is None or len(mesh.faces) == 0:
                # 备选：使用球形初始化
                print("Warning: Point cloud reconstruction failed, falling back to sphere initialization")
                radius = np.linalg.norm(points, axis=1).max() * 0.8
                ...
                get_gt_sdf = func
        elif self.cfg.shape_init.startswith("mesh:") and mesh is not None:
            # 原有的mesh处理逻辑
            # 注释掉所有变换步骤以保留原始mesh
            # # move to center
    ...
    def isosurface(self) -> Mesh:
        ...
        if (
            self.cfg.fix_geometry
            and self.mesh is not None
            and getattr(self, "_cached_isosurface_threshold", None)
            == self.cfg.isosurface_threshold
        ):
            return self.mesh
        level = self.sdf - float(self.cfg.isosurface_threshold)
        mesh = self.isosurface_helper(level, self.deformation)
        ...
        self.mesh = mesh
        self._cached_isosurface_threshold = float(self.cfg.isosurface_threshold)
        return mesh
    ...
    def create_from(
        ...
        **kwargs,
        ...
        def apply_convert_scale(instance: "TetrahedraSDFGrid") -> "TetrahedraSDFGrid":
            if instance.cfg.convert_mesh_scale is None:
                return instance
            scale = float(instance.cfg.convert_mesh_scale)
            if scale != 1.0:
                # Scale bbox (world space) and SDF values to keep distances consistent.
                instance.isosurface_bbox.mul_(scale)
                instance.sdf.data.mul_(scale)
            return instance
        ...
        if isinstance(other, TetrahedraSDFGrid):
            ...
            return apply_convert_scale(instance)
            ...
            instance.isosurface_bbox = mesh.extras["bbox"].clone()
            instance.sdf.data = (
