...
class ImplicitSDF(BaseImplicitGeometry):
    ...
    def configure(self) -> None:
        ...
        # Runtime SDF subtraction applied after sdf_bias.
        # This is controlled by the system (e.g., scheduled by training step).
        self.sdf_subtract: float = 0.0

    def set_sdf_subtract(self, value: float) -> None:
        self.sdf_subtract = float(value)
    ...
    def initialize_shape(self) -> None:
        ...
        if self.cfg.shape_init == "ellipsoid":
            ...
            print(f"Initialize shape with mesh: {mesh_path}, scale: {scale:.6f}")
            # mesh.vertices = mesh.vertices / scale * self.cfg.shape_init_params
            mesh.vertices = mesh.vertices * self.cfg.shape_init_params
        ...
        for step in tqdm(
            ...
            if step % 100 == 0:
                print(f"max|sdf_gt|: {sdf_gt.abs().max().item():.6f}, max|sdf_pred|: {sdf_pred.abs().max().item():.6f}")
                print(f"min|sdf_gt|: {sdf_gt.abs().min().item():.6f}, min|sdf_pred|: {sdf_pred.abs().min().item():.6f}")
                print(f"avg|sdf_gt|: {sdf_gt.abs().mean().item():.6f}, avg|sdf_pred|: {sdf_pred.abs().mean().item():.6f}")
                print(f"Step {step}, loss: {loss.item():.6f}, lr: {scheduler.get_last_lr()[0]:.8f}")

        # explicit broadcast to ensure param consistency across ranks
        for param in self.parameters():
            broadcast(param, src=0)

    def _get_mesh_sdf_bias(
        self, points: Float[Tensor, "*N Di"], mesh_path: str
    ) -> Float[Tensor, "*N 1"]:
        """计算mesh的SDF bias"""
        
        # 缓存机制：避免重复加载同一个mesh
        if not hasattr(self, '_mesh_sdf_cache'):
            self._mesh_sdf_cache = {}
        
        if mesh_path not in self._mesh_sdf_cache:
            # 加载和预处理mesh（复用initialize_shape中的逻辑）
            if not os.path.exists(mesh_path):
                raise ValueError(f"Mesh file {mesh_path} does not exist.")
            ...
            # 预处理mesh（根据sdf_bias_params进行缩放等操作）
            if hasattr(self.cfg, 'sdf_bias_params') and self.cfg.sdf_bias_params is not None:
                if isinstance(self.cfg.sdf_bias_params, dict):
                    # 支持更复杂的预处理参数
                    scale = self.cfg.sdf_bias_params.get('scale', 1.0)
                    center = self.cfg.sdf_bias_params.get('center', False)
                ...
                elif isinstance(self.cfg.sdf_bias_params, float):
                    # 简单缩放
                    mesh.vertices = mesh.vertices * self.cfg.sdf_bias_params
            
            # 创建SDF查询器并缓存
            from pysdf import SDF
            sdf_func = SDF(mesh.vertices, mesh.faces)
            self._mesh_sdf_cache[mesh_path] = sdf_func
        
        # 获取缓存的SDF函数
        sdf_func = self._mesh_sdf_cache[mesh_path]
        ...
        # 查询SDF值
        sdf_values = -sdf_func(points_flat)  # 添加负号以匹配标准SDF约定
        
        # 转换回tensor并恢复形状
        sdf_bias = torch.from_numpy(sdf_values).to(points.device, points.dtype)
        sdf_bias = sdf_bias.reshape(*original_shape[:-1], 1)
    ...
    def get_shifted_sdf(
        ...
        #     # 新增mesh选项
        #     mesh_path = self.cfg.sdf_bias[5:]  # 去掉"mesh:"前缀
        #     sdf_bias = self._get_mesh_sdf_bias(points, mesh_path)
        elif isinstance(self.cfg.sdf_bias, float):
            sdf_bias = self.cfg.sdf_bias
        else:
            raise ValueError(f"Unknown sdf bias {self.cfg.sdf_bias}")
        return sdf + sdf_bias - self.sdf_subtract
