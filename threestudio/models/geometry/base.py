    ...
    def isosurface(self) -> Mesh:
        ...
        if self.cfg.isosurface_coarse_to_fine:
            ...
            if mesh_coarse.v_pos.numel() == 0:
                threestudio.warn(
                    f"Empty isosurface mesh extracted (isosurface_threshold={self.cfg.isosurface_threshold}, radius={self.cfg.radius}). "
                    "This usually means the chosen threshold does not intersect the field within the bbox; "
                    "returning the coarse mesh without bbox tightening."
                )
                return mesh_coarse
