    ...
    def forward(
        ...
        **kwargs
        ...
        if mesh.v_pos.shape[0] == 0 or mesh.t_pos_idx.shape[0] == 0:
            # Return empty output if no geometry
            return {
                "opacity": torch.zeros(batch_size, height, width, 1, device=mesh.v_pos.device),
                "comp_normal": torch.zeros(batch_size, height, width, 3, device=mesh.v_pos.device),
                "sd_comp_normal": torch.zeros(batch_size, height, width, 3, device=mesh.v_pos.device),
                "mask": torch.zeros(batch_size, height, width, 1, device=mesh.v_pos.device),
                "mv_disparity": torch.zeros(batch_size, height, width, 1, device=mesh.v_pos.device),
                "depth": torch.zeros(batch_size, height, width, 1, device=mesh.v_pos.device),
                "mesh": mesh
            }
        ...
        # Fix camera_distances dimensions by removing trailing singleton dimensions
        camera_distances = kwargs["camera_distances"]
        while len(camera_distances.shape) > 1 and camera_distances.shape[-1] == 1:
            camera_distances = camera_distances.squeeze(-1)
        
        near_d = camera_distances - self.cfg.near_distance
        far_d = camera_distances + self.cfg.near_distance
        # print(f"camera_distances: {camera_distances}, near_d: {near_d}, far_d: {far_d}")
        # print(f"shape of camera_distances: {camera_distances.shape}, near_d: {near_d.shape}, far_d: {far_d.shape}")
        ...
        # Apply antialiasing to depth to ensure gradient flow
        depth_aa = self.ctx.antialias(depth, rast, v_pos_clip, mesh.t_pos_idx)
