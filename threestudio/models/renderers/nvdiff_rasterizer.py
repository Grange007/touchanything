...
class NVDiffRasterizer(Rasterizer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # comp_normal_cam: 不混杂背景，只是归一化到[0,1]，类似NeuS
        # -1,1 -> [0,1]
        comp_normal_cam = (gb_normal_cam + 1.0) / 2.0
        comp_normal_cam_aa = self.ctx.antialias(
            comp_normal_cam, rast, v_pos_clip, mesh.t_pos_idx
        )

        # comp_normal_cam_vis: 混杂背景用于可视化
        comp_normal_cam_vis = torch.lerp(bg_normal, comp_normal_cam, mask.float())
        comp_normal_cam_vis_aa = self.ctx.antialias(
            comp_normal_cam_vis, rast, v_pos_clip, mesh.t_pos_idx
        )

        # for bg white normal (用于可视化)
        comp_normal_cam_white_vis = torch.lerp(bg_normal_white, comp_normal_cam, mask.float())
        comp_normal_cam_white_vis_aa = self.ctx.antialias(
            comp_normal_cam_white_vis, rast, v_pos_clip, mesh.t_pos_idx
        )
        ...
        # Add depth output
        depth_masked = torch.lerp(torch.zeros_like(depth_raw), depth_raw, mask.float())
        depth_aa = self.ctx.antialias(depth_masked, rast, v_pos_clip, mesh.t_pos_idx)

        out["depth"] = depth_aa
        out["depth_raw"] = depth_raw
        ...
        out.update(
            {
                ...
                "comp_normal_cam": comp_normal_cam_aa,  # 不混杂背景，纯粹的相机空间法线
                "comp_normal_cam_vis": comp_normal_cam_vis_aa,  # 混杂背景用于可视化
                "comp_normal_cam_white_vis": comp_normal_cam_white_vis_aa,
            }
        )  # in [0, 1]
        ...
        if render_rgb:
            ...
            # # Add SDF output
            # sdf_full = torch.zeros(batch_size, height, width, 1).to(positions)
            # sdf_full[selector] = geo_out["sdf"]
            # out["sdf"] = sdf_full
