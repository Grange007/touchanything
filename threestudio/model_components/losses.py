...
# from threestudio.cameras.rays import RaySamples
...
def monosdf_normal_loss(normal_pred: torch.Tensor, normal_gt: torch.Tensor):
    """normal consistency loss using ONLY 1 - cos

    Args:
        normal_pred (torch.Tensor): volume rendered normal
        normal_gt (torch.Tensor): monocular normal
    """
    # 1. 强制在最后一个维度进行 L2 归一化，确保它们严格成为单位方向向量
    normal_gt = torch.nn.functional.normalize(normal_gt, p=2, dim=-1)
    normal_pred = torch.nn.functional.normalize(normal_pred, p=2, dim=-1)
    
    # 2. 计算 1 - cos loss
    # 此时两个向量的模长均为1，点乘的结果直接等价于它们夹角的余弦值 (cos)
    cos_loss = (1.0 - torch.sum(normal_pred * normal_gt, dim=-1)).mean()
...
def compute_scale_and_shift(prediction, target, mask):
    ...
    valid = det.nonzero()
    print("a_00", a_00, "a_01", a_01, "a_11", a_11, "b_0", b_0, "b_1", b_1, "det", det)
    print("valid", valid)
...
def l1_loss(prediction, target, mask, reduction=reduction_batch_based):
    M = torch.sum(mask, (1, 2))
    res = prediction - target
    image_loss = torch.sum(mask * torch.abs(res), (1, 2))
...
class ScaleAndShiftInvariantLoss(nn.Module):
    def __init__(self, alpha=0.5, scales=4, reduction="batch-based"):
        ...
        # self.__regularization_loss = l1_loss
        self.__alpha = alpha
    ...
    def forward(self, prediction, target, mask):
        ...
        # print("prediction_min", prediction.min(), "prediction_max", prediction.max())
        # print("target_min", target.min(), "target_max", target.max())
